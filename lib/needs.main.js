(function (root) {
	...
	*/
	function _resolve (path, context) {
		...
		*/
		if (~_rem.indexOf(path)) {
			return path;
		}

		if (~path.indexOf("!")) {

			path = path.replace(/([\d,\w,\s,\.\/]*)(?=\!)/, function ($0, $1) {
		...
		require(plugin, function (pluginModule) {
			...
			*/
			...
			load.fromText = function (name, definition, dqL) {
				...
				*/
				q.m[moduleIndex] = pluginPath = plugin + "!" + name;
	...
	function _module (id, def, noExports, ns, i, l, parts, pi) {
		...
		*/
		...
		for (i = 0, l = parts.length; i < l; i ++) {
			pi = parts[i];
			if (!ns[pi] || (!ns[pi].exports && i === l-1) && !def) {
				if (!def) {
					/**
