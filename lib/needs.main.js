(function (root) {
	...
	* Also removes any leading "/" from the path, this is an important note
	* as it behaves slightly different than usual implementations of normalize()
	*/
	...
	*/
	function _resolve (path, context) {
		return _normalize((context || "") + "/" + path);
	}
	...
	*/
	function _load (modules, callback, i, m, f) {
		...
		for (i = 0; i < modules.length; i ++) {
			...
			if (~m.indexOf("!")) {
				...
				*/
				return _loadPluginModule(m, _loadQ.length-1, i);
			}
	...
	/** 
	* Called by _load(), used for loading plugin-type modules
	*/
	function _loadPluginModule (module, qIndex, moduleIndex, plugin, pluginPlusModule) {
		...
		*/
		require(plugin, function (pluginModule) {
			...
			*/
			module = pluginModule.normalize ? pluginModule.normalize(module) : module;
			pluginPlusModule = plugin + "!" + module;

			/**
			* Update the loadQ with the newly computed module id
			*/
			_loadQ[qIndex].m[moduleIndex] = pluginPlusModule;

			/**
			* If the module is already defined, we don't need to do anything else, so return.
			*/
			if (_module(pluginPlusModule)) {
				return;
			}

			function load (definition) {
				_module(pluginPlusModule, {exports: definition});
				_checkLoadQ();
			}

			load.fromText = function (name, definition, dqL, q) {

				/**
				* Update the loadQ with the newly computed module id
				*/
				_loadQ[qIndex].m[moduleIndex] = plugin + "!" + name;
				...
				* to local vars, and UglifyJS does not mangle variables if it finds "eval()" in your code.
				*/
				...
				if(_defineQ.length > dqL) {
					...
					_invokeAnonymousDefine(plugin + "!" + name);
				}
			};

			pluginModule.load(
				module,
				require.localize(null),
				load,
				_config[plugin] || {}
			);
		});
	...
	function require (ids, callback, context) {
		...
		if (!callback) {
			...
			*/
			...
			if (!callback) {
				/**
				* If the module is not defined, throw an error.
				*/
		...
		* Inkoke the callback immediately, swapping "require" with the actual require function
		*/
		callback.apply(root, _swapValues(modules, {"require" : require}));
	}
